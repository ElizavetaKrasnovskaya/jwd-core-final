...
public interface ApplicationMenu {
    ...
    Logger logger = org.slf4j.LoggerFactory.getLogger(ApplicationMenu.class);
    ...
    default void handleUserInput() throws IOException, InvalidStateException {
        ...
        Long previousCrewFileSize = Files.size(Paths.get(NassaContext.getInstance().getCrewPath()));
        Long previousSpaceshipFileSize = Files.size(Paths.get(NassaContext.getInstance().getSpaceshipPath()));
        while (true) {
            if (Duration.between(lastCheckTime, LocalDateTime.now()).toMinutes() > applicationProperties.getFileRefreshRate()) {
                lastCheckTime = LocalDateTime.now();
                if (Files.size(Paths.get(NassaContext.getInstance().getCrewPath())) != previousCrewFileSize
                        || Files.size(Paths.get(NassaContext.getInstance().getSpaceshipPath())) != previousSpaceshipFileSize) {
                    previousCrewFileSize = Files.size(Paths.get(NassaContext.getInstance().getCrewPath()));
                    previousSpaceshipFileSize = Files.size(Paths.get(NassaContext.getInstance().getSpaceshipPath()));
